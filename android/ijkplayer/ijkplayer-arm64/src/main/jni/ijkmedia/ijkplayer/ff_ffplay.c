...
static int read_thread(void *arg)
{
    ...
    if (ffp->iformat_name)
        ...
        AVDictionaryEntry * entry = av_dict_get(ffp->format_opts, "fflags", NULL, 0);
        if (entry)
        {
            
                 av_log(ffp, AV_LOG_WARNING, "DHG AVDictionaryEntry value.%s\n", entry->value);
        }else{
             av_log(ffp, AV_LOG_WARNING, "DHG  else AVDictionaryEntry value\n");
...
void ffp_set_option(FFPlayer *ffp, int opt_category, const char *name, const char *value)
{
    ...
    av_dict_set(dict, name, value, 0);
      AVDictionaryEntry * entry = av_dict_get(*dict, "fflags", NULL, 0);
        if (entry)
        {
            
                 av_log(ffp, AV_LOG_WARNING, "DHG set option AVDictionaryEntry value.%s\n", entry->value);
        }else{
             av_log(ffp, AV_LOG_WARNING, "DHG set option else AVDictionaryEntry value\n");
